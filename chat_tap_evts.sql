...
(
  ...
  select
      ...
      ,case when n_profiles_tapping = 0 and max(cht.connection_id) is not null then 'no tap, chat started'
            ...
            when n_profiles_tapping = 1 and max(cht.connection_id) is not null and max(cht.profile_id) <> max(tap.profile_id) then 'single tap, chat started by tap receiver'
